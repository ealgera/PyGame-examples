...
class Cirkel(pygame.sprite.Sprite):
    ...
    def update(self, sprites):
        ...
        self.vectors = []
        v_center_to_self = self.pos - v_center
        self.vectors.append(v_center_to_self)

        #for cirkel in sprites:  # Bereken de vector tussen 2 sprites
        #   if cirkel == self:   # Niet met zichzelf
        #       continue
        #   else:
        #       v = cirkel.vel - self.vel # - self.vel  # Vector van deze sprite naar een andere
        #       #print(self.vel, cirkel.vel, v)
        #       self.vectors.append(v*10)
    ...
    def draw(self, screen):
        ...
        #    pygame.gfxdraw.line(screen, int(self.pos.x), int(self.pos.y), int(self.pos.x+vector.x), int(self.pos.y+vector.y), RED)
        #    pygame.gfxdraw.filled_circle(screen, int(self.pos.x+vector.x), int(self.pos.y+vector.y), 5, RED)  # Bolletje op lijn   

        for vector in self.vectors:
            pygame.gfxdraw.line(screen, int(v_center.x), int(v_center.y), int(vector.x + WIDTH/2), int(vector.y + HEIGHT/2), WHITE)
...
class Sketch():
    ...
    def run(self):
        ...
        while not done:
            ...
            #self.all_sprites.update(self.all_sprites)          # Voer de .update methode uit voor iedere sprite in de sprites-Group
            ...
            #a = self.all_sprites.draw(self.screen) # 'Teken' alle sprites.
