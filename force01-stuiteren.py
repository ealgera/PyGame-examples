...
class Cirkel(pygame.sprite.Sprite):
    ...
    def update(self, sprites):
        ...
        if (self.pos.x - self.radius) <= 0 or (self.pos.x + self.radius) >= WIDTH:
            self.pos.x = (WIDTH - self.radius) if (self.pos.x+self.radius) > WIDTH else self.pos.x
            self.vel.x = int(self.vel.x * -1)

#        if ((self.pos.y - self.radius) <= 0) or ((self.pos.y + self.radius) >= HEIGHT):
#            self.pos.y = (HEIGHT - self.radius) if (self.pos.y+self.radius) > HEIGHT else self.pos.y
...
class Sketch():
    def __init__(self):
        ...
        self.cirkels = [Cirkel(randint(0,WIDTH), randint(0,HEIGHT*2/3), randint(50,50), str(x) ) for x in range(1, AANTAL_CIRKELS+1)]
